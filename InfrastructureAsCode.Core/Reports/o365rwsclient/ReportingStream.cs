...
namespace InfrastructureAsCode.Core.Reports.o365rwsclient
{
    ...
    public class ReportingStream
    {
        ...
        public ReportingStream(ReportingContext context, string reportType, string streamIdentifier, string progressFilePath = null)
        {
            ...
            this.reportType = Type.GetType("InfrastructureAsCode.Core.Reports.o365rwsclient.TenantReport." + reportType);

            if (!this.reportType.IsSubclassOf(typeof(ReportObject)) || !this.reportType.IsSerializable)
            {
                var emsg = new ArgumentException(string.Format("Report Type Must be Subclass of ReportObject and Serializable: {0}", this.reportType.FullName));
                reportingContext.TraceLogger.LogError(emsg, "Report Type Must be Subclass of ReportObject and Serializable: {0}", this.reportType);
                throw emsg;
            }
        }
